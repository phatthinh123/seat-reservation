...
public class PaymentController {
    ...
    public PaymentResponse pay(@RequestBody PaymentRequest req) {
        ...
        // Check if global simulation flags override the request flags
        boolean actualFail = req.simulateFail() || SimulationController.nextFail.getAndSet(false);
        boolean actualDelay = req.simulateDelay() || SimulationController.nextDelay.get();
        
        payments.put(paymentId, new PaymentRecord(req.bookingId(), PaymentStatus.PENDING, actualFail));
        
        // Deliver webhook (async) - pass actualDelay to the webhook delivery service
        webhookDelivery.scheduleDelivery(paymentId, req.bookingId(), req.callbackUrl(), actualFail, actualDelay);
        
        log.info("Initiated mock payment {} for booking {}, status=PENDING, actualFail={}, actualDelay={}", paymentId, req.bookingId(), actualFail, actualDelay);
    ...
    public record PaymentRequest(String bookingId, String callbackUrl, boolean simulateFail, boolean simulateDelay) {}
